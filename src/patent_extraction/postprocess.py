...
def choose_scalar(values: List[Any], policy: MergePolicy, fallback_policy: MergePolicy = "prefer_non_null") -> Any:
    if not values:
        return None
...
def parse_and_validate(json_str: str) -> PatentMetadata:
    try:
        ...
        print(f"⚠️  Erreur de validation JSON: {exc}")
        print(f"→ JSON brut:\n{json_str}\n")
...
__all__ = [
    ...
    "choose_scalar",
    "compute_confidence",
    "date_coherence_subscore",
    "enforce_date_order",
    "entity_subscore",
    "extract_json",
    "first_non_empty",
    "fix_duplicate_dates",
    "fix_inventor_assignee_confusion",
    "is_company_name",
    "is_missing",
    "longest_non_empty",
    "merge_entity_lists",
